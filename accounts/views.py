...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            #create user using create_user method [in models.py under UserManager class]
...
def registerVendor(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid() and v_form.is_valid():
            ...
            messages.success(request, 'Account is registered successfully! Please wait for the approval.')
...
def login(request):
    if request.user.is_authenticated:
        ...
        if user is not None:
            ...
            messages.success(request, 'Logged in to you account.')
            return redirect('myAccount')
...
def reset_password_validate(request, uidb64, token):
    # Validate the user by decoding the token and user primary key(pk)
    try:
        uid = urlsafe_base64_decode(uidb64).decode()
        user = User._default_manager.get(pk = uid)
    except(TypeError, ValueError,OverflowError, User.DoesNotExist):
